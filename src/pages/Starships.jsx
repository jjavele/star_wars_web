...
export default function Starships() {
  ...
  const starships = useSelector((state) => state.starships.starships.results)
    ...
    const fetchingData = async () => {
      try {
        ...
        if (searchTerm.length > 0) {
          await dispatch(starshipActions.renderSearchedStarships(searchTerm));
        } else {
          await dispatch(starshipActions.renderStarships(count));
        }
      } catch (error) {
        ...
        <div className="flex flex-wrap justify-center md:justify-between w-[85vw]">
          ...
          <div className="flex w-full min-h-[85vh] justify-center items-center">
            ...
            {!isLoading && starships?.map((starship, index) => {
              ...
              return (
                <Anchor key={index} to={`/starship/${starshipId}`}>
