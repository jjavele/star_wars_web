...
export default function Planets() {
  ...
  const filteredResults = combinedResults?.filter((planet) => {
    ...
    for (let i = 0; i < planetName.length; i++) {
      if (planetName[i] === searchLower[matchIndex]) {
        matchIndex++;
      }
  ...
  useEffect(() => {
    api.get(apiURL + endpoints.read_planets + `/?page=${count}`)
      .then((res) => setCombinedResults(res.data.results))
      .catch((error) => console.log(error));
  }, [count]);

  useEffect(() => {
    api.get(apiURL + endpoints.read_planets + `/?search=${searchTerm}`)
      .then((res) => setCombinedResults(res.data.results))
      .catch((error) => console.log(error));
  }, [searchTerm]);
    ...
    <div className="flex flex-wrap w-full min-h-[85vh] justify-center items-center bg-[url('/src/assets/images/background.jpg')] bg-cover">
      ...
      <div className="flex flex-wrap justify-center lg:justify-between w-[85vw]">
        {filteredResults?.map((planet, index) => {
          const planetsId = planet?.url?.match(/\/(\d+)\/$/)[1];
          const imageSrc = `https://starwars-visualguide.com/assets/img/planets/${planetsId}.jpg`;
          return (
            <Anchor to={`/planet/${planetsId}`}>
              <Card key={index} imageSrc={imageSrc} name={planet?.name} planetsId={planetsId} defaultImage={"/src/assets/images/default-planets-image.png"}/>
            </Anchor>
          );
        })}
      </div>
